...
class Data_preprocess(object):
    ...
    def load_labels(self, model):
        ...
        for ind in image_ind:
            ...
            imagename = os.path.join(self.data_path, 'train_image', ind + '.jpg')
    ...
    def load_data(self, index):
        ...
        filename = os.path.join(self.data_path, 'train_annotations', index + '.xml')
        ...
        for obj in objects:
            ...
            #下面的两个判断语句主要是为了排除不合格的标注和极端的标注，必须注意标注数据检查
            if x1>=x2 or y1>=y2:
                continue
            if abs(y2-y1)<6:
                continue
            ...
            if xind>12 or yind>12:
                continue
            #注意下面是先写的yind  h w
            label[yind, xind, :, 0] = 1
            label[yind, xind, :, 1:5] = boxes#中心点格式(x,y,w,h)
